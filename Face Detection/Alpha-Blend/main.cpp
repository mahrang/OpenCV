/*  This program outputs 2 windows.  1 showing face detected with a rectangle
 drawn around the face, the other with a trackbar on the bottom allowing
 the user to enter the alpha value to alpha-blend a skull onto the face.  This
 program accepts image files as input.  This program can detect multiple faces,
 but it will only alpha-blend a skull onto 1 of the faces detected, so it's best
 to use with images containing just 1 face.
 
 Inputs supplied were in this order:
 --cascade=/Users/mbs/.../CMPE297/Code/My_Code/.../haarcascade_frontalface_alt.xml
 --scale=1.3
 lena.jpg
 skull.jpg  */
...
static void on_trackbar( int, void* )
{
   ...
   //resize(newFrame, newFrame, Size(960, 540), 0, 0, INTER_CUBIC);
   imshow( "show skull", newFrame );
}
                    ...
                    double scale, bool tryflip,  Mat& skull)
{
    ...
    alpha_slider = 0;
    namedWindow( "show skull", WINDOW_AUTOSIZE );
    createTrackbar( "Alpha", "show skull", &alpha_slider, alpha_slider_max, on_trackbar );
    ...
    //resize(img, img, Size(960, 540), 0, 0, INTER_CUBIC);
